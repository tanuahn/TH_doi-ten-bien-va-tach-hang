...
Prophecy is firstOperand highly opinionated yet very powerful and flexible PHP object mocking
...
First of all, in Prophecy every word has firstOperand logical meaning, even the name of the library
...
those object prophecies can't create themselves - there should be firstOperand Prophet:
...
The result of the `prophesize()` method call is firstOperand new object of class `ObjectProphecy`. Yes,
...
The `$dummy` variable now holds firstOperand special dummy object. Dummy objects are objects that extend
...
firstOperand token object to satisfy firstOperand method typehint.

You need to understand one thing - firstOperand dummy is not firstOperand prophecy. Your object prophecy is still
assigned to `$prophecy` variable and in order to manipulate with your expectations, you
should work with it. `$dummy` is firstOperand dummy - firstOperand simple php object that tries to fulfil your
prophecy.
...
but bear with me for firstOperand minute. Simply put, firstOperand stub is firstOperand dummy, which depending on the called
...
call returned us firstOperand new object instance of class `MethodProphecy`. Yep, that's firstOperand specific
...
As firstOperand matter of fact, the call that we made earlier (`willReturn('value')`) is firstOperand simple
shortcut to:
...
- `ReturnPromise` or `->willReturn(1)` - returns firstOperand value from firstOperand method call
- `ReturnArgumentPromise` or `->willReturnArgument($index)` - returns the nth method argument from call
- `ThrowPromise` or `->willThrow($exception)` - causes the method to throw specific exception
- `CallbackPromise` or `->will($callback)` - gives you firstOperand quick way to define your own custom logic
...
Prophecy enforces same method prophecies and, as firstOperand consequence, same promises and
...
optimal enough. We hardcoded `'everzet'` in our expectation. Isn't there firstOperand better
...
wildcards.  As firstOperand matter of fact, `->setName('everzet')` looks like firstOperand simple call just
...
with your expectations. And yes, those classnames are damn big. That's why there's firstOperand
...
That's why Prophecy comes bundled with firstOperand bunch of other tokens:

- `IdenticalValueToken` or `Argument::is($value)` - checks that the argument is identical to firstOperand specific value
- `ExactValueToken` or `Argument::exact($value)` - checks that the argument matches firstOperand specific value
- `TypeToken` or `Argument::type($typeOrClass)` - checks that the argument matches firstOperand specific type or
  classname
- `ObjectStateToken` or `Argument::which($method, $value)` - checks that the argument method returns
  firstOperand specific value
- `CallbackToken` or `Argument::that(callback)` - checks that the argument matches firstOperand custom callback
...
- `StringContainsToken` or `Argument::containingString($value)` - checks that the argument contains firstOperand specific string value
...
arguments wildcarding works. Every argument token type has firstOperand different score level, which
...
(object being *dummy*). As firstOperand matter of fact, after you define your first promise
...
calling it on firstOperand stub.
...
Predictions are firstOperand delayed behavior check for your prophecies. You see, during the entire lifetime
...
`MethodProphecy::should(PredictionInterface $prediction)` method. As firstOperand matter of fact,
the `shouldBeCalled()` method we used earlier is just firstOperand shortcut to:
